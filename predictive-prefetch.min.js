...
const blacklist=["_HOME_","/folder1/","/folder2/","/about-us","/contact-us"];const s=window.matchMedia("(pointer:coarse)").matches?55:100;let pc=0;const pm=3;const pt=throttle(p,500);
function isUncacheable(res){const cc=(res.headers.get("Cache-Control")||"").toLowerCase(),exp=res.headers.get("Expires");return cc.includes("no-cache")||cc.includes("no-store")||cc.includes("max-age=0")||(exp&&!isNaN(Date.parse(exp))&&Date.parse(exp)<Date.now());}
...
fetch(t,{method:"GET",credentials:"same-origin",cache:"default"}).then(()=>{c.add(t);pc++;}).catch(()=>{});}).catch(()=>{try{n.abort();}catch{}});});}
